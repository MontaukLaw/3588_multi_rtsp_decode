...
MPP_RET test_mpp_run(MpiEncMultiCtxInfo *info)
{
    ...
    mpp_log("input %s\n", cmd->file_input ? cmd->file_input : "camera");
    mpp_log("output %s\n", cmd->file_output ? cmd->file_output : "null");
    // print p type
    mpp_log("type %d\n", p->type);
    // print p frm eos
    mpp_log("frm_eos %d\n", p->frm_eos);
    ...
    // print hor_stride and ver_stride
    ...
    while (!p->pkt_eos)
    {
        ...
        auto buffer_length = mpp_buffer_get_size(p->frm_buf);
        ...
        else if (p->custom_input)
        {
            printf("get img........");
            cv::Mat rgbframe = pop_out_media();
            cv::Mat yuvframe;
            cv::cvtColor(rgbframe, yuvframe, CV_BGR2YUV_I420);
            // print aligned width and height
            printf("aligned width %d\n", MPP_ALIGN(p->width, 16));
            printf("aligned height %d\n", MPP_ALIGN(p->height, 16));
            memcpy(buf, yuvframe.data, p->height * p->width * 3 / 2);
        }
        else
        {
            if (p->cam_ctx == NULL)
        ...
        if (p->osd_enable || p->user_data_enable || p->roi_enable)
        {
            if (p->user_data_enable)
            {
                ...
                char *str = "this is user g_dec_loop_data\n";
                ...
                char *str1 = "this is user g_dec_loop_data 1\n";
                char *str2 = "this is user g_dec_loop_data 2\n";
                data_group.count = 2;
        ...
        do
        {
            ...
            if (packet)
            {
                ...
                if (p->custom_input)
                {
                    ...
                    printf("streaming....");
                    stream_data(ptr, pkt.size, pkt.dts, pkt.pts);
                }
...
void *enc_test(void *arg)
{
    ...
    if (ret)
    {
        mpp_err_f("test g_dec_loop_data init failed ret %d\n", ret);
        goto MPP_TEST_OUT;
    }
...
MPP_RET mpi_enc_test_cmd_setup(MpiEncTestArgs *cmd, int width, int height)
{
    RK_S32 ret = MPP_OK;
    RK_U32 i;
    ...
    cmd->file_input = nullptr;
    cmd->file_output = nullptr;
...
void *mpi_enc_test(int width, int height)
{
    ...
    ret = mpi_enc_test_cmd_setup(cmd, width, height);
    if (ret)
        goto DONE;
    mpp_log("hor_stride %d\n", cmd->hor_stride);
    mpp_log("ver_stride %d\n", cmd->ver_stride);
    ...
    ret = enc_test_multi(cmd, "mpi_enc_test");
