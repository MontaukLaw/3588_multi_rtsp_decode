...
// int decode_simple(MpiDecLoopData *data, void* pdata, int len, uint32_t pts, uint32_t dts)
{
    RK_U32 pkt_done = 0;
    RK_U32 pkt_eos = 0;
    RK_U32 err_info = 0;
    ...
    MppCtx ctx = data->ctx;
    MppApi *mpi = data->mpi;
    // char   *buf = g_dec_loop_data->buf;
    MppPacket packet = NULL;
    MppFrame frame = NULL;
    size_t read_size = 0;
    size_t packet_size = data->packet_size;

    // 首先是将av packet中的数据拷贝到 mpp 的packet中
    ...
    do
    {
        ...
        if (!pkt_done)
        {
            mpp_log("decode_simple read packet\n");
            // 把packet扔给mpp进行解码
            ret = mpi->decode_put_packet(ctx, packet);
            if (MPP_OK == ret)
            {
                pkt_done = 1;
                mpp_log("decode_simple read packet success\n");
            }
            else
            {
                mpp_err("decode_simple read packet failed\n");
                break;
            }
        }
        else
        {
            mpp_log("decode_simple flush\n");
        }
        ...
        try_again:
            ...
            if (frame)
            {
                ...
                else
                {
                    ...
                    {
                        cv::Mat rgbImg;
                        // 把frame转为rgb格式，用于yolov5推理
                        YUV420SP2Mat(frame, rgbImg);
                        // 放入图片buffer中
                        push_src_media(rgbImg);
                    }
                }
            ...
            {
                mpp_log("decode_get_frame found no frame\n");
            }
